# CLI analysis logic (Object). Kept separate from SceneTree entry so Godot 3.4
# can -s a thin main script; large SceneTree mains can hang before _initialize.
...
func _parse_args() -> Dictionary:
	...
	# PoolStringArray on Godot 3.4 may lack Array.find — copy into a plain Array.
	var args = []
	...
	if OS.has_method("get_cmdline_user_args"):
		...
		for ui in range(ua.size()):
			user_args.append(ua[ui])
	if user_args.size() == 0:
		var dash_index = -1
		...
		if dash_index >= 0:
			...
			# Godot 3.0/3.1 may omit the "--" sentinel; still accept our --flags.
			for i in range(args.size()):
				var a = str(args[i])
				if a == "--project-path" or a == "--output" or a == "--csv-output" or a == "--html-output" or a == "--history-path" or a == "--baseline" or a == "--diff" or a == "--fail-on-diff-regression" or a == "--no-fail-on-threshold" or a == "--fail-on-threshold" or a == "--min-r2" or a == "--metrics-out" or a == "--apply" or a == "--step":
					user_args.append(a)
					if i + 1 < args.size() and not str(args[i + 1]).begins_with("-"):
						user_args.append(args[i + 1])

	var i = 0
	while i < user_args.size():
		var arg = str(user_args[i])
		var key = arg
		var val = ""
		var eq = arg.find("=")
		if eq > 0 and arg.begins_with("--"):
			key = arg.substr(0, eq)
			val = arg.substr(eq + 1, arg.length() - (eq + 1))
		if key == "--project-path":
			if val != "":
				project_path = _sanitize_path(val)
				i += 1
			elif i + 1 < user_args.size():
			...
			if val != "":
				output_path = _sanitize_path(val)
				i += 1
			elif i + 1 < user_args.size():
			...
			if val != "":
				csv_output_path = _sanitize_path(val)
				i += 1
			elif i + 1 < user_args.size():
				...
				i += 1
		elif key == "--html-output":
			if val != "":
				html_output_path = _sanitize_path(val)
				i += 1
			elif i + 1 < user_args.size():
			...
			if val != "":
				history_path = _sanitize_path(val)
				i += 1
			elif i + 1 < user_args.size():
			...
			if val != "":
				baseline_path = _sanitize_path(val)
				i += 1
			elif i + 1 < user_args.size():
			...
			do_diff = true
			i += 1
		elif key == "--fail-on-diff-regression":
			fail_on_diff_regression = true
			i += 1
		elif key == "--no-fail-on-threshold":
			fail_on_threshold = false
			i += 1
		elif key == "--fail-on-threshold":
